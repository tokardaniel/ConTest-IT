...
@step('Eszközök rögzítése a következő id-val rendelkező partnerhez: "{id}"')
...
@step("Rögzített eszközök exportálása excelben")
def step_impl(c):
    Eszkozok.download(c)

@step('A következő id-val rendelkező "{id}" partnerhez tartozó egyik eszköz szervízstátuszának módosítása')
def step_impl(c, id: int):
    q = Query()
    partner: Partner = q.get_partner_by_test_id(id=int(id))
